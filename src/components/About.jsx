...
import { FaArrowUp, FaS } from "react-icons/fa6"
...
export default function About() {
   ...
   function handleOnMouseEvent(isEntering) {
      ...
      gsap.to('img', {scale: isEntering ? (window.innerWidth > 1280 ? 1.08 : 0.9) : 1, duration: 1})
   }
      ...
      <div className="flex mb:flex-col px-1 justify-between mb:gap-5 border-t border-gray-700 py-20 mb:py-2">
           <div className="flex flex-col p-1 gap-5 ml-10 tb:ml-0 mb:ml-0">
             <span className="text-6xl tb:text-4xl mb:text-3xl">Our approach:</span>
              <button onMouseEnter={() => handleOnMouseEvent(true)} onMouseLeave={() => handleOnMouseEvent(false)} className="flex px-1 py-3 mb:p-2 bg-gray-900 gap-1 text-white justify-evenly items-center text-base w-1/2 tb:w-3/4 mb:w-[60%] rounded-full">
                  READ MORE 
